...
export default async function RiderPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gruposPersonal: Record<string, typeof proyecto.personal> = {};
  for (const p of proyecto.personal) {
    const tipo = p.participacion ?? "OPERACION";
    if (!gruposPersonal[tipo]) gruposPersonal[tipo] = [];
    gruposPersonal[tipo].push(p);
  }

  let transporte: { vehiculoId: string; choferId: string; comentarios: string }[] = [];
  try { transporte = proyecto.transportes ? JSON.parse(proyecto.transportes) : []; } catch { transporte = []; }
    ...
    <>
      ...
      <div className="page" style={{ maxWidth: 820, margin: "0 auto", padding: "32px 40px" }}>
        ...
        <div style={{ borderBottom: "2px solid #111", paddingBottom: 16, marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, color: "#B3985B", fontWeight: 700, marginBottom: 4 }}>MAINSTAGE · FICHA TÉCNICA DE PRODUCCIÓN</div>
        ...
        {proyecto.personal.length > 0 && (
          <>
            <div className="section-title">Personal técnico</div>
            {Object.entries(gruposPersonal).map(([tipo, grupo]) => (
              <div key={tipo} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#555", marginBottom: 4 }}>
                  {participacionLabels[tipo] ?? tipo}
                </div>
                <div style={{ border: "1px solid #e8e8e8", borderRadius: 4, overflow: "hidden" }}>
                  {grupo.map((p, idx) => (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "6px 12px", borderBottom: idx < grupo.length - 1 ? "1px solid #f0f0f0" : "none", background: idx % 2 === 0 ? "white" : "#fafafa" }}>
                      <div style={{ width: 12, height: 12, border: "1.5px solid #ccc", borderRadius: 2, flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
                        <span style={{ fontSize: 11, fontWeight: p.tecnico ? 600 : 400, color: p.tecnico ? "#111" : "#aaa" }}>
                          {p.tecnico?.nombre ?? "Pendiente de asignar"}
                        </span>
                        {(p.rolTecnico?.nombre) && (
                          <span style={{ fontSize: 10, color: "#888", marginLeft: 6 }}>{p.rolTecnico.nombre}</span>
                        )}
                      </div>
                      {p.fechaJornada && (
                        <span style={{ fontSize: 9, color: "#aaa" }}>{new Date(p.fechaJornada + "T12:00:00Z").toLocaleDateString("es-MX", { timeZone: "UTC", weekday: "short", day: "numeric", month: "short" })}</span>
                      )}
                      {p.tecnico?.celular && (
                        <span style={{ fontSize: 10, color: "#666" }}>{p.tecnico.celular}</span>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {proyecto.equipos.length > 0 && (
          <>
            <div className="section-title">Rider de carga</div>
              ...
              <div key={cat} style={{ marginBottom: 12 }}>
                ...
                <div style={{ border: "1px solid #ddd", borderTop: "none", borderRadius: "0 0 3px 3px" }}>
                  {items.map((e, idx) => {
                    ...
                    const librarySugs = e.equipo.accesorios.filter(a => !riderNames.has(a.nombre.toLowerCase()));
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: "1px solid #ddd", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div style={{ fontSize: 9, color: "#aaa" }}>
            ...
            <div style={{ marginTop: 2 }}>Mainstage — Ficha Técnica de Producción</div>
          </div>

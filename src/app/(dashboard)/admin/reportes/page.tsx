...
interface RentabilidadData {
  proyectos: RentProyecto[];
  porTipoEvento: RentTipoEvento[];
  kpis: {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        <div>
          <h2 className="text-base font-bold text-white">Rentabilidad por Evento</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="ms-card px-5 py-4">
              ...
              <p className={`text-2xl font-bold tabular-nums ${rentMargenColor(data.kpis.margenGlobal)}`}>
                {rentFmtPct(data.kpis.margenGlobal)}
              </p>
            </div>
          </div>
                ...
                {data.porTipoEvento.map(t => (
                  <div key={t.tipoEvento} className="px-4 py-3 flex items-center gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1">
                        <div className="h-1.5 bg-[#1e1e1e] rounded-full flex-1">
                          <div
                            className={`h-full rounded-full ${rentMargenBg(t.margenPromedio)}`}
                    ...
                    <div className="text-right shrink-0">
                      <p className={`text-sm font-bold tabular-nums ${rentMargenColor(t.margenPromedio)}`}>{rentFmtPct(t.margenPromedio)}</p>
                ...
                <div key={p.id} className="px-4 py-4 hover:bg-[#141414] transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col items-center shrink-0 w-14">
                      <span className={`text-sm font-bold tabular-nums ${rentMargenColor(p.margen)}`}>{rentFmtPct(p.margen)}</span>
                      <div className="h-1 bg-[#1e1e1e] rounded-full w-full mt-1">
                        <div
                          className={`h-full rounded-full ${rentMargenBg(p.margen)}`}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                        ...
                        {p.fechaEvento && (
                          <p className="text-[10px] text-gray-600">{rentFmtFecha(p.fechaEvento)}</p>
                        )}
                      </div>
...
type Tab = "balance" | "flujo" | "asistencias" | "rentabilidad";
      ...
      <div className="print-layout" id="print-report" style={{ background: 'white', color: 'black', padding: '0', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #B3985B', paddingBottom: 16, marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Módulo: {tab === 'balance' ? 'Balance General' : tab === 'flujo' ? 'Flujo de Caja' : tab === 'rentabilidad' ? 'Rentabilidad por Evento' : 'Asistencias'}</div>
          </div>

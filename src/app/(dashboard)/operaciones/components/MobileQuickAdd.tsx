...
const MobileQuickAdd = forwardRef<MobileQuickAddHandle, Props>(function MobileQuickAdd(
  ...
  // Expose focus() to parent so FAB can call it synchronously (iOS keyboard trick)
  useImperativeHandle(ref, () => ({
    focus: () => titleRef.current?.focus(),
  }));

  // iOS visual viewport: keep sheet above keyboard
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {/* Expandible panel — inside scroll area so it never pushes chips off screen */}
          {panel && (

...
export default function BodegaPage() {
  ...
  async function crearSemana() {
    ...
    const d = await r.json();
    setCreando(false);
    if (d.checklist?.id) router.push(`/inventario/bodega/${d.checklist.id}`);
  }
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5">
        ...
        {checklistSemana ? (
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => router.push(`/inventario/bodega/${checklistSemana.id}`)}
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {checklists.filter(c => c.semana !== semanaActual).map(c => {
              ...
              return (
                <button key={c.id} onClick={() => router.push(`/inventario/bodega/${c.id}`)}

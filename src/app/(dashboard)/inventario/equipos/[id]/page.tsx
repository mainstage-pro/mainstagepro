...
type GaleriaFoto = { url: string; nombre: string; uso: "INTERNO" | "EXTERNO" };

function parseGaleria(raw: string | null): GaleriaFoto[] {
  if (!raw) return [];
  try {
...
function GaleriaSection({ equipoId, initial }: { equipoId: string; initial: string | null }) {
  const toast = useToast();
    ...
    <div className="ms-card p-5">
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[10px] text-[#555] mt-1">Fotos del equipo en eventos · <span className="text-emerald-500">Externo</span> = uso en cotizaciones/presentaciones · <span className="text-sky-500">Interno</span> = montaje</p>
        </div>
        <button
...
export default function EquipoFichaPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      {/* Galería */}
      <GaleriaSection equipoId={equipo.id} initial={equipo.imagenesUrls} />

        ...
        <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-[#1a1a1a]">
          ...
          {cot.estado === "ENVIADA" && (
            <button
              ...
              disabled={saving}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-medium text-red-400 hover:text-red-300 border border-red-900/30 hover:border-red-800/50 rounded-lg transition-colors"
            >
              ✕ Rechazada
            </button>
          )}
      ...
      <div className="ms-stat-card">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
          {ESTADOS_FLUJO.map((e, idx) => {
            const isActive = e === cot.estado;
            const isPast   = ESTADOS_FLUJO.indexOf(cot.estado) > idx && cot.estado !== "RECHAZADA";
            return (
              <button key={e} disabled={saving || isActive} onClick={() => cambiarEstado(e)}
                className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors border ${
                  isActive
                    ? ESTADO_COLORS[e] + " border-transparent"
                    : isPast
                    ? "bg-[#0d0d0d] text-gray-600 border-[#1a1a1a] cursor-pointer hover:text-white"
                    : "bg-[#1a1a1a] text-gray-500 hover:text-white border-[#2a2a2a] hover:border-[#444]"
                }`}>
                {isPast && <span className="text-emerald-600 text-[10px]">✓</span>}

        ...
        <div className="relative z-10 py-32 px-6 text-center max-w-3xl mx-auto">
          ...
          <R delay={260}>
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
              <BotonDescarga
                url={`/api/cotizaciones/${cotizacion.id}/pdf${token ? `?token=${token}` : ""}`}
              ...
              {cotizacion.tratoId && (
                <>
                  ...
                  <BotonDescarga
                     url={`/api/contratos/${cotizacion.tratoId}/pdf${token ? `?token=${token}&cotizacionId=${cotizacion.id}` : ""}`}
                     filename={`Contrato-${cotizacion.numeroCotizacion}.pdf`}
                     titulo="Contrato"
                     className="flex items-center gap-2 text-white/25 hover:text-white/50 text-xs transition-colors">

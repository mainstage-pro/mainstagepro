...
import { Wine, Music, Building2, Sparkles, type LucideIcon } from "lucide-react";
...
const TABS: { key: string; label: string; sub: string; icon: LucideIcon }[] = [
  { key: "SOCIAL", label: "Sociales", sub: "Bodas · XV años · Fiestas", icon: Wine },
  { key: "MUSICAL", label: "Musicales", sub: "Conciertos · Festivales · DJ", icon: Music },
  { key: "EMPRESARIAL", label: "Empresariales", sub: "Congresos · Lanzamientos", icon: Building2 },
  { key: "OTRO", label: "Otros eventos", sub: "Deportivos · Teatro · Comedia", icon: Sparkles },
];
...
function PaquetesClientInner({ defaultTab }: { defaultTab?: string } = {}) {
  ...
  const inicial = defaultTab && TIPOS_VALIDOS.includes(defaultTab) ? defaultTab : null;
  const [tab, setTab] = useState<string | null>(inicial);
  ...
  const productosTab = useMemo(
    () => (tab ? productos.filter((p) => parseJSON(p.tiposEvento).includes(tab)).slice(0, 8) : []),
    [productos, tab]
  );
      ...
      <section className="mx-auto max-w-6xl px-6 pt-32 pb-10 sm:pt-40">
        ...
        <p className="mt-6 max-w-xl text-base leading-relaxed text-white/50 sm:text-lg">
          {tab
            ...
            : "Elige el tipo de evento para ver sus paquetes armados."}
        </p>
      </section>
      ...
      {!tab ? (
        ...
        <section className="mx-auto max-w-6xl px-6 pb-32">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {TABS.map((t, i) => (
              <R key={t.key} delay={i * 70}>
                <button onClick={() => setTab(t.key)}
                  className="group w-full flex items-center gap-5 p-7 rounded-3xl text-left transition-all hover:-translate-y-1"
                  style={{ border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
                  <span className="shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-colors"
                    style={{ background: "rgba(179,152,91,0.12)", border: `1px solid ${GOLD}33` }}>
                    <t.icon strokeWidth={1.75} className="w-6 h-6" style={{ color: GOLD }} />
                  </span>
                  <span className="flex-1">
                    <span className="block font-semibold text-white text-lg mb-1">{t.label}</span>
                    <span className="block text-white/40 text-sm">{t.sub}</span>
                  </span>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                    className="shrink-0 text-white/25 transition-all group-hover:text-white group-hover:translate-x-1">

...
// TEMP — Single-use endpoint to create Trade pilot test record
// DELETE this file after seeding

const SECRET = "mainstage-piloto-2025";
const COT_NUM = "COT-2026-TRADE-01";
const TRADE_TOKEN = `trade-test-${Date.now()}`;
...
export async function POST(req: NextRequest) {
  ...
  const trato = await prisma.trato.create({
    data: {
      ...
      ideasReferencias: "https://pinterest.com/example — Boda elegante con pantallas LED laterales y ambiente íntimo",
  ...
  const subtotalEquiposNeto = 142000;
  const subtotalOperacion = 18500;
  const subtotalTransporte = 4800;
  const subtotalComidas = 2400;
  const subtotalHospedaje = 3200;
  const total = subtotalEquiposNeto + subtotalOperacion + subtotalTransporte + subtotalComidas + subtotalHospedaje;
  ...
  const tradeData = {
    ...
    entregables: [
      ...
      "Técnico de mezcla de audio con experiencia en bodas de alto perfil",
    ],
    checklist: {},
  ...
  const cot = await prisma.cotizacion.create({
    data: {
      ...
      observaciones: "Propuesta generada para piloto Mainstage Trade. Cliente calificado Priority.",
      ...
      lineas: {
        create: [
          {
            categoria: "AUDIO",
            descripcion: "Sistema Line-Array L-Acoustics KARA II (8+2 elementos por lado)",
            cantidad: 1,
          ...
          {
            categoria: "AUDIO",
            descripcion: "Consola de mezcla Yamaha CL5 + técnico especializado",
            cantidad: 1,
          ...
          },
          {
            ...
            descripcion: "Moving heads Claypaky Sharpy 300 (12 unidades)",
            cantidad: 1,
          ...
          {
            categoria: "ILUMINACION",
            descripcion: "Wash LED RGBW Elation Fuze Wash 700 (16 unidades) — arquitectónica",
            cantidad: 1,
          ...
          {
            categoria: "VIDEO",
            descripcion: "Pantallas LED 3x4m laterales con procesador Novastar (2 módulos)",
            cantidad: 2,
            precioUnitario: 18000,
            subtotal: 36000,
          },

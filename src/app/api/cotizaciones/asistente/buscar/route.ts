...
export async function POST(req: NextRequest) {
  ...
  // Filtro opcional: "EQUIPO" | "ROL" para acotar según el tipo de la línea.
  ...
  if (!soloRol) {
    ...
    for (const a of accesorios) {
      candidatos.push({
        kind: "ACCESORIO", equipoId: null, rolTecnicoId: null,
  ...
  if (!soloEquipo) {
    ...
    for (const r of roles) {
      candidatos.push({
        kind: "ROL", equipoId: null, rolTecnicoId: r.id,

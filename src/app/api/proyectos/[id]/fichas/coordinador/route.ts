...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Resolve vehiculo and chofer names for transportes
  ...
  const data: FichaCoordinadorData = {
    ...
    proveedoresRenta,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    archivos: (proyecto.archivos ?? []).map((a: any) => ({
      tipo: a.tipo,
      nombre: a.nombre,
      url: a.url,
    })),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    checklist: (proyecto.checklist ?? []).map((c: any) => ({
      item: c.item,
      completado: c.completado,
      tipo: c.tipo,
    })),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    cuentasCobrar: (proyecto.cuentasCobrar ?? []).map((c: any) => ({
      concepto: c.concepto,
      monto: c.monto,
  ...
  const pdfStream = await ReactPDF.renderToStream(
    React.createElement(FichaCoordinador, { data }) as React.ReactElement<React.ComponentProps<typeof Document>>
  );

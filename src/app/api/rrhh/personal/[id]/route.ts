import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getSession } from "@/lib/auth";
import { calcularSaldo } from "@/lib/vacaciones";

export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const { id } = await params;

  // Últimos ~4 meses de asistencia para el expediente (no toda la historia).
  const desdeAsistencia = new Date();
  desdeAsistencia.setMonth(desdeAsistencia.getMonth() - 4);

  const persona = await prisma.personalInterno.findUnique({
    where: { id },
    include: {
      documentos: { orderBy: { createdAt: "desc" } },
      pagos: {
        orderBy: { createdAt: "desc" },
        include: { cuentaOrigen: { select: { nombre: true } } },
      },
      jefe: { select: { id: true, nombre: true, puesto: true } },
      asistencias: {
        where: { fecha: { gte: desdeAsistencia } },
        orderBy: { fecha: "desc" },
      },
      evaluaciones: { orderBy: { fecha: "desc" } },
      actas: {
        orderBy: { fecha: "desc" },
        include: { tipo: { select: { nombre: true, gravedad: true } } },
      },
      incidencias: {
        orderBy: { fecha: "desc" },
        include: { tipo: { select: { nombre: true, gravedad: true, categoria: true } } },
      },
      vacaciones: { orderBy: { fechaInicio: "desc" } },
    },
  });

  if (!persona) return NextResponse.json({ error: "No encontrado" }, { status: 404 });

  const saldoVacaciones = calcularSaldo(persona.fechaIngreso, persona.vacaciones);

  // Candidatos a jefe: otros empleados activos (para el selector del formulario).
  const posiblesJefes = await prisma.personalInterno.findMany({
    where: { activo: true, id: { not: id } },
    select: { id: true, nombre: true, puesto: true },
    orderBy: { nombre: "asc" },
  });

  return NextResponse.json({ persona, saldoVacaciones, posiblesJefes });
}

export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const { id } = await params;
  const body = await req.json();

  const allowed = ["nombre", "puesto", "departamento", "tipo", "telefono", "correo", "salario", "periodoPago", "fechaIngreso", "activo", "cuentaBancaria", "datosFiscales", "notas", "banco", "clabe", "numeroCuenta", "numeroTarjeta", "ineUrl", "domicilio", "emergenciaNombre", "emergenciaTel", "padecimientos", "userId", "puestoId", "rfc", "curp", "nss", "fechaNacimiento", "estadoCivil", "fotoUrl", "fechaBaja", "motivoBaja", "jefeId"];
  const fechas = new Set(["fechaIngreso", "fechaNacimiento", "fechaBaja"]);
  const data: Record<string, unknown> = {};
  for (const k of allowed) {
    if (k in body) {
      if (fechas.has(k)) data[k] = body[k] ? new Date(body[k]) : null;
      else if (k === "salario") data[k] = body[k] ? parseFloat(body[k]) : null;
      else data[k] = body[k] === "" ? null : body[k];
    }
  }

  // Al dar de baja, sella la fecha automáticamente si no viene una explícita.
  if (body.activo === false && !("fechaBaja" in body)) {
    const actual = await prisma.personalInterno.findUnique({ where: { id }, select: { fechaBaja: true } });
    if (!actual?.fechaBaja) data.fechaBaja = new Date();
  }

  const persona = await prisma.personalInterno.update({ where: { id }, data });
  return NextResponse.json({ persona });
}

export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const { id } = await params;
  await prisma.personalInterno.delete({ where: { id } });
  return NextResponse.json({ ok: true });
}

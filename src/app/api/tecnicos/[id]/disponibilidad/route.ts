...
/**
 * GET /api/tecnicos/[id]/disponibilidad?fecha=YYYY-MM-DD&proyectoId=XXX
 * Verifica si un técnico tiene conflictos en la fecha dada.
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!fecha) return NextResponse.json({ disponible: true, conflictos: [] });

  // Buscar proyectos donde este técnico está asignado en la misma fecha
  ...
  return NextResponse.json({
    ...
    conflictos: conflictos.map(c => c.proyecto),
  });
}

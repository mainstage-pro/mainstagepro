...
export default function SatisfaccionSelfServicePage() {
  ...
  const submit = async () => {
    ...
    const r = await fetch("/api/rrhh/satisfaccion-publica", {
      ...
      body: JSON.stringify({ personalId, periodo, ...scores, probabilidadRecomendar: nps, loMejor, loMejorable, comentarios }),
    });
    if (r.ok) {
      setEnviado(true);
    } else {
  ...
  if (enviado) return (
    <div style={{ fontFamily: FONT }} className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6">
      <div className="text-center max-w-sm">
        ...
        <h1 className="text-white text-xl font-bold mb-2">¡Gracias, {seleccionado?.nombre}!</h1>
    ...
    <div style={{ fontFamily: FONT }} className="min-h-screen bg-[#0a0a0a] py-10 px-4">
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="space-y-2 max-h-[60vh] overflow-y-auto">
          ...
          {filtrados.length === 0 && (
            <p className="text-center text-[#444] text-sm py-6">Sin resultados</p>
          )}
        </div>
      </div>
    </div>
    ...
    <div style={{ fontFamily: FONT }} className="min-h-screen bg-[#0a0a0a] py-8 px-4">
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="text-center pt-4 pb-2">
          <p className="text-[#B3985B] text-xs uppercase tracking-widest font-semibold mb-3">Mainstage Pro</p>
          <h1 className="text-white text-2xl font-bold mb-1">Encuesta de Satisfacción</h1>
          <p className="text-[#555] text-sm">{seleccionado.nombre} · {seleccionado.puesto}</p>
          {periodo && <p className="text-[#333] text-xs mt-1">Período: {periodo}</p>}
          <button
            onClick={() => { setPersonalId(""); setBusqueda(""); }}
            className="text-[#B3985B] text-xs mt-2 hover:underline"
          >
            ¿No eres tú? Cambiar
          </button>
        </div>

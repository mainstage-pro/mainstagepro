...
export async function reseedContenidoNichos(): Promise<{ adicionales: number; preguntas: number }> {
  ...
  // 2) Reemplazo de preguntas de nicho (reglas caen por CASCADE). Las generales y
  //    las de tipo (jerarquía) se preservan: solo se recrean las de alcance='nicho'.
  await prisma.preguntaDescubrimiento.deleteMany({ where: { alcance: "nicho" } });
  let ordenP = 0;
  for (const [slug, cont] of Object.entries(NICHO_CONTENIDO)) {
...
// ── Preguntas base de la jerarquía (generales + "Otros eventos") ──────────────
// 17 generales en tres bloques (A lugar/logística, B alcance técnico, C operación
// y comercial) que se preguntan en TODO evento antes de las de tipo/nicho, más 7
// para el tipo "otro" (Producción a la medida), que no tenía preguntas. La 8 es la
// pregunta ancla (selección múltiple) que decide qué ramas se abren; no se parte.
type PreguntaBaseSeed = {
  ref: string;
  texto: string;
  tipoRespuesta: string;
  opciones?: string[];
  alcance: "general" | "tipo";
...
export const PREGUNTAS_GENERALES_SEED: PreguntaBaseSeed[] = [
  // Bloque A · Lugar y logística
  ...
  // Bloque C · Operación y comercial
...
export const PREGUNTAS_OTROS_SEED: PreguntaBaseSeed[] = [
...
/**
 * Siembra idempotente de las preguntas generales y de "Otros eventos". Upsert por
 * texto exacto (no duplica). Resuelve preguntaPadreId por ref en una segunda pasada.
 * Las preguntas generales/tipo NO las borra reseedContenidoNichos (solo toca nicho).
 */
export async function seedPreguntasBase(): Promise<{ generales: number; otros: number }> {
  await ensureCatalogoTables();
  const todas = [...PREGUNTAS_GENERALES_SEED, ...PREGUNTAS_OTROS_SEED];

  const existentes = await prisma.preguntaDescubrimiento.findMany({ select: { id: true, texto: true } });
  const idPorTexto = new Map(existentes.map(p => [p.texto, p.id]));
  const idPorRef = new Map<string, string>();

  // Base de orden por encima de las de nicho para dejarlas al final del arreglo.
  const maxOrden = (await prisma.preguntaDescubrimiento.aggregate({ _max: { orden: true } }))._max.orden ?? 0;
  let orden = maxOrden + 1;

  // Pasada 1: crear/actualizar sin el padre (aún no conocemos todos los ids).
  for (const q of todas) {
    const data = {
      texto: q.texto,
      tipoRespuesta: q.tipoRespuesta,
      opciones: q.opciones ? JSON.stringify(q.opciones) : null,
      nichos: null,
      alcance: q.alcance,
      tipoEventoSlug: q.alcance === "tipo" ? (q.tipoEventoSlug ?? null) : null,
      bloque: q.alcance === "general" ? (q.bloque ?? null) : null,
      obligatoria: !!q.obligatoria,
      activa: true,
    };
    const existenteId = idPorTexto.get(q.texto);
    if (existenteId) {
      await prisma.preguntaDescubrimiento.update({ where: { id: existenteId }, data });
      idPorRef.set(q.ref, existenteId);
    } else {
      const creada = await prisma.preguntaDescubrimiento.create({ data: { ...data, orden: orden++ } });
      idPorRef.set(q.ref, creada.id);
    }
  }

  // Pasada 2: enlazar condicionales por ref.
  for (const q of todas) {
    if (!q.padreRef) continue;
    const id = idPorRef.get(q.ref);
    const padreId = idPorRef.get(q.padreRef);
    if (id && padreId) {
      await prisma.preguntaDescubrimiento.update({
        where: { id },
        data: { preguntaPadreId: padreId, condicionValor: q.condicionValor ?? null },
      });
    }
  }

  return { generales: PREGUNTAS_GENERALES_SEED.length, otros: PREGUNTAS_OTROS_SEED.length };
}

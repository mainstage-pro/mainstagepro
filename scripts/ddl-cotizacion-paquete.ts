...
async function main() {
  await sql.query(`ALTER TABLE cotizaciones ADD COLUMN IF NOT EXISTS "paqueteId" TEXT`);
  const t = await sql.query(
    `SELECT column_name FROM information_schema.columns WHERE table_name = 'cotizaciones' AND column_name = 'paqueteId'`
  );
  console.log("OK columna paqueteId presente:", t.length > 0);
}

// DDL aditivo en prod: cotizaciones.idioma / cotizaciones.traduccionEn.
...
async function main() {
  await sql.query(`ALTER TABLE cotizaciones ADD COLUMN IF NOT EXISTS "idioma" TEXT NOT NULL DEFAULT 'es'`);
  await sql.query(`ALTER TABLE cotizaciones ADD COLUMN IF NOT EXISTS "traduccionEn" TEXT`);
  const cols = await sql.query(`
    SELECT column_name FROM information_schema.columns
    WHERE table_name = 'cotizaciones' AND column_name IN ('idioma','traduccionEn')
